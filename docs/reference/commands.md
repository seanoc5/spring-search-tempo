...
**Access application**: http://localhost:8085
...
# Run built image
docker run -p 8085:8085 com.oconeco/spring-search-tempo
```
...
# Batch jobs
curl http://localhost:8085/actuator/batch
```
...
| Trigger NLP | `curl -X POST http://localhost:8085/api/nlp/process`        |
