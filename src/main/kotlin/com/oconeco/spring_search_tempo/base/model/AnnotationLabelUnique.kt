...
@MustBeDocumented
@Constraint(validatedBy = [AnnotationLabelUniqueValidator::class])
annotation class AnnotationLabelUnique(
    val message: String = "{Exists.annotation.label}",
    val groups: Array<KClass<*>> = [],
    val payload: Array<KClass<out Payload>> = []
)


class AnnotationLabelUniqueValidator(
    private val annotationService: AnnotationService,
    private val request: HttpServletRequest
) : ConstraintValidator<AnnotationLabelUnique, String> {
    ...
    override fun isValid(`value`: String?, cvContext: ConstraintValidatorContext): Boolean {
        ...
        if (currentId != null && value.equals(annotationService.get(currentId.toLong()).label,
                ignoreCase = true)) {
            // value hasn't changed
            return true
        }
        return !annotationService.labelExists(value)
    }
